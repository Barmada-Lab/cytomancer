...
@click.option("--well-csv", type=click.Path(exists=True, path_type=pl.Path), help="CSV file with well information")
def cli_entry(project_name: str, experiment_dir: pl.Path, well_csv: pl.Path | None):
    ...
    if (upload_record_path := results_dir / "cvat_upload.csv").exists():
        ...
        raise ValueError(f"Could not find {upload_record_path}; was this project uploaded with an older version of cytomancer? If so, use the legacy survival script.")
