...
@click.option(
    "--channel", "-c", default="DAPI", show_default=True, help="Channel to segment."
)
@click.option(
    "--label-name",
    "-l",
...
@click.option(
    ...
    default=0.01,
    show_default=True,
    help="Clip limit for adaptive histogram equalization.",
)
@click.option(
    "--median-filter-d",
    "-m",
    default=5,
    show_default=True,
    help="Diameter of the median filter to apply to the images before segmentation.",
)
@click.option(
    "--model-name",
    "-m",
    ...
    show_default=True,
    help="Name of predefined StarDist model to use.",
)
...
def register(cli: click.Group):
    @cli.group(
        "oneoffs",
        help="A place for scripts that I wrote once and might be useful again",
    )
    @click.pass_context
