...
def make_synthetic_nuc_cyto_data(
    gfp_nuc_i: int = 64, gfp_cyto_i: int = 32, dapi_nuc_i: int = 128
):
    blank = np.zeros((2000, 2000), dtype="uint8")
    soma = draw_random_circles(10, 100, blank).astype(bool)
    dapi = (
        morphology.binary_erosion(soma, morphology.disk(30)).astype("uint8")
        * dapi_nuc_i
    )
    ...
    return xr.DataArray(
        images, dims=["time", "region", "field", "channel", "y", "x"], coords=coords
    )
...
def measure_synthetic_nuc_cyto_data(output_dir: Path):
    ...
    task_df = pd.read_csv(
        output_dir / "synthetic_nuc_cyto_task.csv",
        dtype=dtype_spec,
        parse_dates=["time"],
    )
